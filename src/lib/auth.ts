...
module.exports = {
    ...
    isNotVendedorOrAdminOrClient(req:any, res:Response,next:NextFunction){
        if(req.user.Descripcion.toString().trim() === 'Vendedor' || req.user.Descripcion.toString().trim() === 'Administrador' || req.user.Descripcion.toString().trim() === 'Gerente' || req.user.Descripcion.toString().trim() === 'Usuario'){
            return next();
        }
        else{
            return res.redirect('/perfil/');
